	...
	public function index($action=null,$id=null)
	{
		...
		switch ($action) {
			...
			case 'create':
				...
				$data['clinic_id'] 		= $this->app_lib->main_clinic()->id;
				if ($id!=null) {
					$data['row'] 		= $this->model->shops_inventory_row($id);
					$data['clinic_id']  = $data['row']->shop_id;
					$data['sub_categories']	= $this->app_lib->categories($data['row']->parent_cat_id);
					$data['action_url']	  	= base_url('inventory/update/'.$id);
					// echo _prx($data['row']);
					// die();					
				}
			...
			case 'save':
				...
				if ($this->input->server('REQUEST_METHOD')=='POST') {
					...
					$this->form_validation->set_rules('product_id', 'Medicine', 'required');
			...
			case 'update':
				...
				if ($this->input->server('REQUEST_METHOD')=='POST') {
					...
					if ($saved == 1 ) {
						...
						$log_data['qty'] 				= $qty;
						$log_data['qty_change'] 		= $qty - $row->qty;
