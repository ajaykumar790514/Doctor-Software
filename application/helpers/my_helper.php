...
if (!function_exists('PatCount')) {
    function PatCount($clinicId) {
        $CI =& get_instance();
        $CI->load->model('model');
        $patient = $CI->model->getData('patients',['is_deleted'=>'NOT_DELETED','active'=>'1','clinic_id'=>$clinicId]);
        $patientCount = count($patient) ? count($patient) : 0;
        return $patientCount;
    }
}
...
function is_time_slot_available($clinic, $start_time, $end_time, $date)
{
    $CI =& get_instance();
    $CI->load->database();
    ...
    // Check if the time slot clashes with existing appointments
    $CI->db->where('appointment_date', $date)
           ->where('clinic_id', $clinic)
           ->group_start()
               ->where("'$start_datetime' < appointment_end_time", null, false)
               ->where("'$end_datetime' > appointment_start_time", null, false)
           ->group_end();
    $query = $CI->db->get('appointments');

    return $query->num_rows() == 0; // Return true if no clashes, false otherwise
}
